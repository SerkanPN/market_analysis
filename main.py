...
def analyze_store():
    ...
    try:
        ...
        # İstek başlığına, isteğin mağaza sayfasından geldiğini söyleyen "Referer" ekliyoruz
        dynamic_headers = HEADERS.copy()
        dynamic_headers['Referer'] = store_url
        # --------------------------------

        shop_detail_url = f'https://shopee.co.id/api/v4/shop/get_shop_detail?username={username}'
        shop_response = requests.get(shop_detail_url, headers=dynamic_headers) # Güncellenmiş başlığı kullan
        ...
        items_response = requests.get(items_url, headers=dynamic_headers) # Güncellenmiş başlığı kullan
